...
function validateId() {
	txtId.value = txtId.value.replace(/ /g,'');

	var xhr = new XMLHttpRequest();

	xhr.open("GET", "api-contest.php?id=" + txtId.value);
	...
	xhr.onload = function() {
		if(xhr.status == 200) {
			...
			if(txtId.value.length < 3 || data.error === undefined ) {
				txtId.style.borderColor = "rgba(255,0,0,0.5)";
				vId = false;
			}
			else {				
...
function storeDataInDB(key, data) {
	...
	var url = "api-create-contest.php?id=" + key + "&data=" + encodeURIComponent(data);
	...
	xhr.onload = function() {
		if(xhr.status == 200) {
			...
			if(data.result == "success") {
				showMessage("Concurso creado exitosamente!", "success", function() {
					setTimeout(function() {
				...
				showMessage("No se pudo crear el concurso debido a un error.", "error");
			}
		}
	}
	xhr.send();
...
window.addEventListener("load", function() {
	...
	btnCreate.addEventListener("click", createContest);
});